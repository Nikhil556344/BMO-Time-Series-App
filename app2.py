...
if uploaded_file is not None:
    ...
    if 'Date' not in data.columns or 'Close' not in data.columns:
        ...
        if model_choice == "ARIMA":
            ...
            try:
                model_ets = SimpleExpSmoothing(train['Close']).fit(smoothing_level=0.2)
        ...
        # Optional: Matplotlib Forecast Plot
        fig2, ax2 = plt.subplots(figsize=(10, 5))
        ax2.plot(test.index, test['Close'], label='Actual')
        ax2.plot(forecast.index, forecast, label='Forecast')
        ax2.set_title(f'{model_choice} Forecast vs Actual')
        ax2.legend()
        st.pyplot(fig2)
